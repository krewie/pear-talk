...
loop(Table) -> 
	...
	{client, login, ID, Netinfo, Password, ClientPid} ->
		...
		true -> 
		    ...
		    ClientPid ! {db, friendlist, [ID|Netinfo], {ID, ShownName, Friendlist}};
		false ->
		    io:format("Bad password\n", []),
		    ClientPid!{db, badPass, [ID|Netinfo]}; %% fel lösenord
		{error, {badmatch, ID}} ->
		    io:format("Bad username\n", []),
		    ClientPid!{db, badID, [ID|Netinfo]} %% användare existerar inte
	    end,
	    loop(Table);
	...
	{client, adduser, ID, Name, Password,Netinfo,Pid} -> 
		...
		false ->
		    ...
		    Pid ! {db, adduser, [ID|Netinfo], ID, Password};
		true ->
		    Pid !{db, usedID, [ID|Netinfo]}
	    end,
	    loop(Table);
