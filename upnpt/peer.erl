...
start(G) ->
	...
	Me = "guest",
	Vsn = Me,
	ServerAddressS = io:get_line("Insert the name of server to connect: ") -- "\n",
	ServerName = read_address(ServerAddressS),
	

	register(chat, spawn(peer, status, [[{network_interface, localhost},
					     {id,{Me, Vsn}}, {server_name, ServerName}, 
					     {ping_mode, pingoff}, {graphic, G}]])),
...
status(Status) ->
	...
	{login, {Username, Password}}  ->
	    try
		 exit(whereis(upnp),kill)
	    catch
			_:_ ->
				[]
	    end,
	       NewPort = upnp:server(Username),
	       case NewPort of
	       		error ->
	       			Newstatus = Status;
	       		_->
	       			register(chat_server, spawn(peer, server, [NewPort])),
	       			Newstatus = lists:keystore(listen_port, 1, Status, {listen_port, NewPort})
	       end,
	    ...
	    status(lists:keystore(id, 1, Newstatus, {id, {Username,Username}}));
	newfriends ->
	    rul:friends(),
	...
	{ok, Binary} ->
		...
		case Mode of
			...
			if
				...
				case Obj of
				    badPass ->				    
					dialog:msgbox("wrong password!"),
					logout();
				    _ ->
					dialog:msgbox("wrong username!"),
					logout()
				end;
			    true ->
				logout()
...
send(Receiver_username, Mode, Obj) ->
	case rul:peek(friends, Receiver_username, age) of
		...
		[];
	    _ ->
		upnp!{message, Receiver_username, {Mode, Obj}}
	end.
...
send_to_server(Obj) ->
    upnp!{message, my(server_name), Obj}.
    
logout()->
			G = my(graphic),
    		shut_down(),
    		spawn(?MODULE, start,[G]).
